...
if __name__ == '__main__':
    ...
    model_path = './models/'
    model_name = 'MeshSNet_G' # need to define
    checkpoint_name = 'latest_checkpoint_G.tar'
    ...
    print("CUDA Possible :{}".format(torch.cuda.is_available()))
    ...
    Net = MeshSegNet_G(num_classes=num_classes, num_channels=num_channels, with_dropout=True, dropout_p=0.5).cuda()
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.no_grad():
            ...
            for i_batch, batched_val_sample in enumerate(val_loader):
                ...
                A_U = batched_val_sample['A_L'].to(device, dtype=torch.float)
                # Remove missing values
                inputs[inputs!=inputs] = 0
                A_S[A_S!=A_S] = 0
                A_L[A_L!=A_L] = 0

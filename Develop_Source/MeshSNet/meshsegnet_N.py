...
def get_graph_feature(x, k=20, idx=None, dim9=False):
    ...
    if idx is None:
        if dim9 == False:
            ...
            idx = knn(x[:, 6:], k=k)
    idx_base = torch.arange(0, batch_size,device=x.get_device()).view(-1, 1, 1)*num_points
    idx = idx + idx_base
    idx = idx.view(-1)
    _, num_dims, _ = x.size()
...
class MeshSegNet(nn.Module):
    def __init__(self, num_classes=15, num_channels=15, with_dropout=True, dropout_p=0.5):
        ...
        # MLP-2
        # self.mlp3_conv1 = torch.nn.Conv1d(64, 64, 1)
        # self.mlp3_bn1 = nn.BatchNorm1d(64)
        # self.mlp3_conv2 = torch.nn.Conv1d(64, 128, 1)
        # self.mlp3_bn2 = nn.BatchNorm1d(128)
        # self.mlp3_conv1 = torch.nn.Conv1d(512, 512, 2)
        # self.mlp3_bn1 = nn.BatchNorm1d(512)
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(15, 64, kernel_size=1, bias=False),
                                   self.bn1,
    ...
    def forward(self, x, a_s, a_l,a_el):
        ...
        # x = F.relu(self.mlp3_bn2(self.mlp3_conv2(x)))
        # x_mlp3 = F.relu(self.mlp3_bn1(self.mlp3_conv1(x)))
        # if self.with_dropout:
        #     x_mlp3 = self.dropout(x_mlp3)
        # GLM-3
        x = x.transpose(2, 1)
        ...
        # print(x.shape, glm_3_sap_1.shape, glm_3_sap_2.shape, glm_3_sap_3.shape)
        ...
        g = self.conv5(g)
        g1 = F.adaptive_avg_pool1d(g, 1).view(batchsize, -1)
        g2 = F.adaptive_avg_pool1d(g, 1).view(batchsize, -1)
        g = torch.cat((g1, g2), 1)

        g = F.leaky_relu(self.bn6(self.linear1(g)), negative_slope=0.2)
        g = self.dp1(g)
        # Dense fusion
        print(x.shape, x_ftm.shape, x_mlp2.shape, x_glm3.shape,g.shape)
        x = torch.cat([x, x_ftm, x_mlp2, x_glm3,g], dim=1)
        # MLP-4
...
if __name__ == '__main__':
    ...
    summary_(model, [(15, 6000), (6000, 6000), (6000, 6000), (6000, 6000)])
...
def makeh5(data,h5_path):
    if os.path.isfile(h5_path+data[25:]+'.h5'):
        ...
        N_S = np.zeros([6000, 6000], dtype='float32')
        ...
        for i in D:
            ...
            minimum = np.sort(i)
            for j in minimum:
                index_of_minimum = np.where(i == j)
                
                if map(np.float32,Y_train[k]) != map(np.float32,Y_train[index_of_minimum]):
                    index_of_label = index_of_minimum
                    break
                else:
                    continue
            N_S[k,index_of_label] = Label[index_of_label]
            k += 1
        ...
        f.create_dataset('N_S', data=torch.from_numpy(N_S))
        f.close()

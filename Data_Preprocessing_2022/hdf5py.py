...
def makeh5(data,h5_path):
    ...
    if os.path.isfile(h5_path+data[21:-3]+'h5'):
        print(data)
    else:
        i_mesh = data #vtk file name
        print(i_mesh)
        # read vtk
        mesh = load(i_mesh)
        ...
        #create one-hot map
    #        label_map = np.zeros([mesh.cells.shape[0], num_classes], dtype='int32')
    #        label_map = np.eye(num_classes)[labels]
    #        label_map = label_map.reshape([len(labels), num_classes])
        ...
        X_train = np.zeros([7000, X.shape[1]], dtype='float32')
        Y_train = np.zeros([7000, Y.shape[1]], dtype='int32')
        S1 = np.zeros([7000, 7000], dtype='float32')
        S2 = np.zeros([7000, 7000], dtype='float32')
        ...
        if num_positive > 7000: # all positive_idx in this patch
            positive_selected_idx = np.random.choice(positive_idx, size=7000, replace=False)
            selected_idx = positive_selected_idx
        else:   # patch contains all positive_idx and some negative_idx
            num_negative = 7000 - num_positive # number of selected gingiva cells
    ...
    #        mesh2.to_vtp('tmp.vtp')
        ...
        S1[D<0.1] = 1.0
        S1 = S1 / np.dot(np.sum(S1, axis=1, keepdims=True), np.ones((1, 7000)))

        S2[D<0.2] = 1.0
        S2 = S2 / np.dot(np.sum(S2, axis=1, keepdims=True), np.ones((1, 7000)))

        X_train = X_train.transpose(1, 0)
        Y_train = Y_train.transpose(1, 0)

        f = h5py.File(h5_path+i_mesh[21:-3]+'h5','w')
        f.create_dataset('cells', data=torch.from_numpy(X_train))
        f.create_dataset('labels', data=torch.from_numpy(Y_train))
        f.create_dataset('A_S', data=torch.from_numpy(S1))
        f.create_dataset('A_L', data=torch.from_numpy(S2))
        f.close()
    
import h5py
h5_path='\\\\ADS-SHARE\\ADS_Share\\Data\\h5data(7000)\\'
for i in os.listdir('augmentation_vtk_data'):
    makeh5('augmentation_vtk_data/'+i,h5_path)
...
#        self.filename = obj_filename
        ...
        reader.SetFileName(str(obj_filename))
        reader.Update()
        self.reader = reader
...
#        self.filename = vtp_filename
    ...
    def set_cell_labels(self, label_dict, tol=0.005):
        '''
        update:
        ...
        for i_label in label_dict:
            ...
            data = np.argwhere(D<=tol)
            if len(data) > i_label_cell_centers.shape[0]:
                sys.exit('tolerance ({0}) is too large, please adjust.'.format(tol))
            elif len(data) < i_label_cell_centers.shape[0]:
...
#                vtkPolyData.GetCellData().SetVectors(cell_attribute)
    ...
    def to_obj(self, obj_filename):
        for i_label in np.unique(self.cell_attributes['Label']-1):
            with open("./vtptoobj/"+obj_filename+str(int(i_label))+'.obj', 'w') as f:
